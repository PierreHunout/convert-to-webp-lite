	...
	public function prepare( int $attachment_id, array $metadata, array $result = [] ): array {
		...
		try {
			...
			if ( ! is_int( $attachment_id ) || $attachment_id <= 0 ) {
				throw new InvalidArgumentException( __( 'Invalid attachment ID provided.', 'poetry-convert-to-webp' ) );
			}

			// Check if metadata is an array and not empty
			if ( ! is_array( $metadata ) || empty( $metadata ) ) {
				throw new InvalidArgumentException( __( 'Invalid metadata provided.', 'poetry-convert-to-webp' ) );
			}
			...
			if ( false === $filesystem ) {
				throw new RuntimeException( __( 'Failed to initialize WordPress filesystem.', 'poetry-convert-to-webp' ) );
			}
			...
			if ( empty( $file ) || ! $filesystem->exists( $file ) ) {
				...
				throw new RuntimeException( wp_kses( sprintf( __( 'File does not exist for attachment ID: %s', 'poetry-convert-to-webp' ), '<span>' . esc_html( $attachment_id ) . '</span>' ), $allowed_html ) );
			}
			...
			if ( ! in_array( $mime_type, [ 'image/jpeg', 'image/png', 'image/gif' ], true ) ) {
				...
				throw new RuntimeException( wp_kses( sprintf( __( 'Unsupported file type: %s', 'poetry-convert-to-webp' ), '<span>' . esc_html( $mime_type ) . '</span>' ), $allowed_html ) );
			}
			...
			$message = (string) sprintf(
				...
				__( '[Poetry Convert to WebP] Invalid argument in conversion preparation: %s', 'poetry-convert-to-webp' ),
				$error->getMessage()
			);

			if ( (bool) get_option( 'poetry_convert_to_webp_debug_mode', true ) ) {
			...
			$message = (string) sprintf(
				// translators: %s is the error message
				__( '[Poetry Convert to WebP] Runtime error in conversion preparation: %s', 'poetry-convert-to-webp' ),
				$error->getMessage()
			);
			...
			$message = (string) sprintf(
				...
				__( '[Poetry Convert to WebP] Unexpected error preparing conversion: %1$s in %2$s on line %3$d', 'poetry-convert-to-webp' ),
				$error->getMessage(),
	...
	public function convert( string $filepath, ?string $size = null ): array {
		...
		try {
			...
			if ( ! is_string( $filepath ) || empty( $filepath ) ) {
				throw new InvalidArgumentException( __( 'Invalid file path provided.', 'poetry-convert-to-webp' ) );
			}
			...
			if ( ! $filesystem->exists( $filepath ) ) {
				...
				throw new RuntimeException( wp_kses( sprintf( __( 'File does not exist: %s', 'poetry-convert-to-webp' ), '<span>' . esc_html( $filepath ) . '</span>' ), $allowed_html ) );
			}
			...
			if ( empty( $pathinfo['dirname'] ) || empty( $pathinfo['filename'] ) ) {
				...
				throw new RuntimeException( wp_kses( sprintf( __( 'Unable to parse file path: %s', 'poetry-convert-to-webp' ), '<span>' . esc_html( $pathinfo['basename'] ) . '</span>' ), $allowed_html ) );
			}
			...
			if ( $is_webp ) {
				...
				throw new RuntimeException( wp_kses( sprintf( __( 'The original file is already a WebP file: %s', 'poetry-convert-to-webp' ), '<span>' . esc_html( $pathinfo['basename'] ) . '</span>' ), $allowed_html ) );
			}
			...
			if ( $filesystem->exists( $path ) ) {
				...
				throw new RuntimeException( wp_kses( sprintf( __( 'WebP file already exists: %s', 'poetry-convert-to-webp' ), '<span>' . esc_html( $pathinfo['filename'] ) . '.webp</span>' ), $allowed_html ) );
			}
			...
			switch ( strtolower( $pathinfo['extension'] ) ) {
				...
				default:
					// translators: %s is the MIME type of the unsupported file
					$message = (string) wp_kses( sprintf( __( 'Unsupported file type: %s', 'poetry-convert-to-webp' ), '<span>' . esc_html( $pathinfo['basename'] ) . '</span>' ), $allowed_html );
					return Helpers::get_message( false, $message, $this->process, $size ?? '' );
			}

			// Get quality setting from plugin options
			$quality = (int) get_option( 'poetry_convert_to_webp_quality', 85 );

			// If image resource creation failed
			if ( false === $webp ) {
				// translators: %s is the basename of the file for which image resource creation failed
				$message = (string) wp_kses( sprintf( __( 'Failed to create image resource: %s', 'poetry-convert-to-webp' ), '<span>' . esc_html( $pathinfo['basename'] ) . '</span>' ), $allowed_html );
				return Helpers::get_message( false, $message, $this->process, $size ?? '' );
			}
			...
			if ( imagewebp( $webp, $path, $quality ) ) {
				...
				$message = (string) wp_kses( sprintf( __( 'Successfully converted: %s', 'poetry-convert-to-webp' ), '<span>' . esc_html( $pathinfo['basename'] ) . '</span>' ), $allowed_html );
				...
				$message = (string) wp_kses( sprintf( __( 'Failed to save WebP file: %s', 'poetry-convert-to-webp' ), '<span>' . esc_html( $pathinfo['filename'] ) . '.webp</span>' ), $allowed_html );
				return Helpers::get_message( false, $message, $this->process, $size ?? '' );
			}
		} catch ( InvalidArgumentException $error ) {
			$message = (string) sprintf(
				// translators: %s is the error message
				__( '[Poetry Convert to WebP] Invalid argument in file conversion: %s', 'poetry-convert-to-webp' ),
				$error->getMessage()
			);
			...
			$message = (string) sprintf(
				// translators: %s is the error message
				__( '[Poetry Convert to WebP] Runtime error in file conversion: %s', 'poetry-convert-to-webp' ),
				$error->getMessage()
			);
			...
			$message = (string) sprintf(
				// translators: %1$s is the error message, %2$s is the filename, %3$d is the line number
				__( '[Poetry Convert to WebP] Unexpected error converting file: %1$s in %2$s on line %3$d', 'poetry-convert-to-webp' ),
				$error->getMessage(),
				basename( $error->getFile() ),
				$error->getLine()
			);
